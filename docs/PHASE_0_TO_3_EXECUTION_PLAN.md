...
Phase 3 이후 본격적으로 손댈 후보는 아래와 같다. 실행 계획은 별도 문서 [PHASE_4_EXECUTION_PLAN.md](/D:/Work/Projects/BlueFrog/docs/PHASE_4_EXECUTION_PLAN.md)로 분리됐다.
...
Phase 4 v1은 위 후보 중 **텍스처/머티리얼 일반화 + 간단 조명**과 **씬 데이터 직렬화** 두 트랙으로 스코프가 확정됐고, 모두 완료됐다. 단계별 순서·파일·검증 기준은 [PHASE_4_EXECUTION_PLAN.md](/D:/Work/Projects/BlueFrog/docs/PHASE_4_EXECUTION_PLAN.md)를 본다.

Phase 5는 위에서 "씬 데이터 직렬화"가 깐 JSON 로더 위에 **프리팹 + 다중 씬 + 트리거 + 로더 검증**을 얹고 완료됐다. 실행 계획: [PHASE_5_EXECUTION_PLAN.md](/D:/Work/Projects/BlueFrog/docs/PHASE_5_EXECUTION_PLAN.md).

Phase 6은 그 트리거를 **실제 소비자**(이벤트 계층, `ObjectiveState`, 씬 전환)에 연결한다. 실행 계획: [PHASE_6_EXECUTION_PLAN.md](/D:/Work/Projects/BlueFrog/docs/PHASE_6_EXECUTION_PLAN.md).
