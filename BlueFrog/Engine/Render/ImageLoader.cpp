...
namespace ImageLoader
{
    ...
    Surface LoadSurfaceFromMemory(const std::uint8_t* data, std::size_t size)
    {
        ...
        // WIC's IWICStream::InitializeFromMemory expects a non-const pointer
        // even though it is read-only in practice; const_cast is safe here.
        Microsoft::WRL::ComPtr<IWICStream> stream;
        if (FAILED(factory->CreateStream(&stream)))
        {
            throw std::runtime_error("ImageLoader: failed to create WIC stream");
        }
        if (FAILED(stream->InitializeFromMemory(const_cast<BYTE*>(data), static_cast<DWORD>(size))))
        {
            throw std::runtime_error("ImageLoader: failed to init WIC stream from memory");
        }
        ...
        if (FAILED(factory->CreateDecoderFromStream(
                ...
                WICDecodeMetadataCacheOnLoad, &decoder)))
        {
            throw std::runtime_error("ImageLoader: failed to open in-memory image (unsupported format?)");
        }

        Microsoft::WRL::ComPtr<IWICBitmapFrameDecode> frame;
        if (FAILED(decoder->GetFrame(0u, &frame)))
        {
            throw std::runtime_error("ImageLoader: failed to get frame from memory");
        }
        ...
        if (FAILED(converter->Initialize(frame.Get(), GUID_WICPixelFormat32bppRGBA,
                                         ...
                                         WICBitmapPaletteTypeCustom)))
        {
            throw std::runtime_error("ImageLoader: failed to convert in-memory image to RGBA");
        }
