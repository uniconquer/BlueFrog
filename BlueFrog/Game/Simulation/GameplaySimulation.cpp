...
std::wstring GameplaySimulation::BuildWindowTitle(const HudState& hudState) noexcept
{
	...
	oss << L" | " << hudState.objectiveText << L" | WASD move | Space dash | LMB attack | Q/E orbit | Wheel zoom | F1 gizmos | F2 inspector | F3 grid | F5 reload | F8 save profile | F12 save scene";
	return oss.str();
}
