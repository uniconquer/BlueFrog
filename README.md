...
실행하면 `Blue Frog` 창이 뜨고, 제목 표시줄에는 경과 시간과 프레임 시간이 갱신되며 화면에는 회전하는 컬러 삼각형과 애니메이션 배경이 표시됩니다.
...
- [BlueFrog/Core/Renderer.cpp](/D:/Work/Projects/BlueFrog/BlueFrog/Core/Renderer.cpp): 테스트 지오메트리와 렌더 파이프라인
...
- `BeginFrame/Draw/EndFrame` 구조와 테스트 삼각형 렌더링 추가
- 테스트 렌더링 경로를 `Renderer` 클래스로 분리
